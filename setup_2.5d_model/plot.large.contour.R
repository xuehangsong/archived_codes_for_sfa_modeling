...
level.value = level.value[!names(level.value) %in% c("SWS-1","NRG")]
...
{
    print(itime)
    jpeg(paste("figures/","level",itime,".jpg",sep=''),width=6.5,height=6.0,units='in',res=200,quality=100)
    par(mar=c(5.1,6,4.1,10))
    level.snap = unlist(lapply(level.value,"[[",itime))
    grid.level = interp(coord.data[names(level.value),1][!is.na(level.snap)],
                        coord.data[names(level.value),2][!is.na(level.snap)],
                        level.snap[!is.na(level.snap)],
##                        linear=FALSE,
...
##                        extrap=TRUE
    ...
    filled.contour(grid.level$x,
                   ...
                   zlim=c(104.5,105.5),
                   asp = 1,
##                   key.title =title(main="Water level (m)",line=0),
                   plot.axes={
                       axis(1)
                       axis(2)
...
#    mtext(side = 3,at=594250,format(interp.time[itime],format="%Y-%m-%d   %H:%M",tz="GMT"),line=1,cex=2)
    mtext(side = 3,at=594120,"Water level (m)",line=0.5,cex=1.2)
