...
spc.temp.value = spc.temp.value[!names(spc.temp.value) %in% c("SWS-1","NRG","S10","S40","T2","T3","T4","T5")]
...
{
    print(itime)
    jpeg(paste("figures/","spc.temp",itime,".jpg",sep=''),width=6.5,height=6.0,units='in',res=200,quality=100)
    par(mar=c(5.1,6,4.1,10))
    spc.temp.snap = unlist(lapply(spc.temp.value,"[[",itime))
    grid.spc.temp = interp(coord.data[names(spc.temp.value),1][!is.na(spc.temp.snap)],
                        coord.data[names(spc.temp.value),2][!is.na(spc.temp.snap)],
                        spc.temp.snap[!is.na(spc.temp.snap)],
##                        linear=FALSE,
...
##                        extrap=TRUE
    ...
    filled.contour(grid.spc.temp$x,
                   grid.spc.temp$y,
                   grid.spc.temp$z,
...
##    mtext(side = 3,at=594250,format(interp.time[itime],format="%Y-%m-%d   %H:%M",tz="GMT"),line=1,cex=2)
    mtext(side = 3,at=594120,expression(paste('Temperature ('^"o","C)")),line=0.5,cex=1.2)
