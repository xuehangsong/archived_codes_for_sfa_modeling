...
##         points(bottomleft.point[1],bottomleft.point[2])         
         ...
         left.points = cbind(data["2-2",1]+clx,data["2-2",2]-cly)
         right.points = cbind(topleft.point[1]+clx,topleft.point[2]-cly)
         right.points = right.points[order(right.points[,2]),]
         left.points = left.points[order(left.points[,2]),]
         ...
         ipoint=1
         for (inode in c(1,2,4,8,9,11,15,18,23))             
         {
             ...
             fm <- lm(y~x,dfm)
             line.points.x = data.frame(x=seq(594400,594560,0.01))
...
##             points(line.points.x,line.points.y)
