...
##         angle = -70.04518+180
         ...
         required.depth = c(103.5,104.5,105.5)
         nsection = 20
         ...
         tube.start = array(0,c(nsection,2))
         tube.points = array(0,c(nsection*ndepth,2))         
         tube.start[1,] = data["goldman_south",]
         tube.start[2,1] = (data["array_south",1]-8*cos(angle/180*pi) + data["goldman_south",1])/2
         tube.start[2,2] = (data["array_south",2]-8*sin(angle/180*pi) + data["goldman_south",2])/2
         tube.start[3,1] = tube.start[1,1]+190*cos(angle/180*pi)
         tube.start[3,2] = tube.start[1,2]+190*sin(angle/180*pi)
         tube.start[4,1] = data["1-10A",1]
         tube.start[4,2] = data["1-10A",2]
...
##             lines(line.points.x,line.points.y,)
         ...
         ## for (isecion in seq(4,20,5))
         ## {
         ##     points(tube.points[isecion*3+c(1:3),],cex=1,pch=16,col="cyan")         
         ## }
