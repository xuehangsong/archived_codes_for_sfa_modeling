...
def def_release_particle_from_river_bed(river_particle_dy, river_particle_dz):
    ...
    new_particles = []
    for iy in range(river_particle_ny):
        pline_selected = unique_face_index[:, unique_face_index[1, :] ==
                                           river_particle_y_index[iy]]
        for icell in range(pline_selected.shape[1]):
            if (pline_selected[2, icell] in river_particle_z_index) and (
                ...
                new_particles.append(
                    np.array([times[itime], x[pline_selected[0, icell]],
...
material_file = (
    "./flow_2008/BC_2008_2015/" +
    "bigplume_4mx4mxhalfRes_material_mapped_newRingold_subRiver_17tracer.h5")
...
if caculate_path:
    ...
    for itime in range(0, ntime - 1):
        ...
        if times[itime] in release_times:
            ...
            #                    [times[itime], 317, 99, 95]).reshape((1, 4)),
            ...
            # define new particle locations
            new_particles = def_release_particle_from_river_bed(
        ...
        for iparticles in range(len(active_particles)):
            ...
            particles[particle_index] = np.append(
                particles[particle_index],
                results[iparticles]["coord_update"], 0)
            particle_status[particle_index] = results[
                iparticles]["particle_status"]
...
if plot_path:
    ...
    if plot_surface:
        ...
        river_surf_z = griddata(
            (river_face_x, river_face_y), river_face_z,
    ...
    # for imaterial in range(9,10):
    # ##material_type["Ringold_Fine"]:##,material_type["Ringold_Fine"]:
