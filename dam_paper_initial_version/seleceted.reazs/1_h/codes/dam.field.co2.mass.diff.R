...
smooth = h5data[[ivari]]-zero[[ivari]]


load(paste("1/","h5data2920.r",sep=""))
zero=h5data
load(paste("1/","h5data17528.r",sep=""))

base = h5data[[ivari]]-zero[[ivari]]



data = log10(abs(base-smooth))

## data[which(log10(smooth)<=-6)]=NA
## data[which(log10(base)<=-6)]=NA

zlim = c(-6,2)
## data[data < zlim[1]] = NA
## data[data > zlim[2]] = NA
jpg.name = paste("download.figure/",ireaz,"/diff",ivari,".jpg",sep = "")
...
filled.contour(x,z,data,
               ...
               xlim = c(80,143.2),
               ylim = c(98.5,110),
##               key.axes = axis(4,c(-3,-1.5,0,1.5,3)),               
               asp=1,

               ##               main = paste(ivari,"   ",format(output.time[files[ifile]+1],"%Y-%m-%d %H:%M")),
##               color.palette = function(x)rev(cm.colors(x)),
