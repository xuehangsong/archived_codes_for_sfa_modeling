...
jpg.name = paste("download.figure/stage.alluvium",ivari,"_",ireaz,"_.pdf",sep = "")
##jpeg(jpg.name,width = 3.3,height = 3.3,units = "in",res = 300, quality = 100)
pdf(jpg.name,width = 3.3,height = 3.3)
...
smoothScatter(stage,log10(base)-log10(compare),
              ...
              xlab="",              
              asp=1,
              ylim=c(-4,4),xlim=c(104,109),              
              colramp=colorRampPalette(c("white","blue")))
box()
axis(1)
axis(2,las=2)
mtext("River stage (m)",1,1.5)
mtext(expression(paste("Diff. of C CONS rate (log10, mol/m"^"2","S)"),sep=""),2,1.5)
dev.off()
