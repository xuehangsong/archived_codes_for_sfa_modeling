...
for (i.line in c(2,3,4,5,7,6))
{
    thermistor.data[[i.line]]=thermistor.data[[i.line]]-thermistor.data[[6]]
}
...
jpeg(filename="thermistor.difference.jpeg",width=10,height=8,units='in',res=200,quality=100)
...
mtext(side=2,text="Temperature difference with river(DegC)",line=3)
...
legend("topleft",c(names(thermistor.data)[2:7],"WL"),
       lty=c(line.lty,1),
       lwd=c(line.lwd,1),
       col=c(line.col,"blue"),
       bty='n'
