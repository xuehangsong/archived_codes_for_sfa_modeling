...
# fname_mvAwln = "/Users/shua784/Dropbox/PNNL/People/From_Patrick/SQL/mvAwln_since_2009.csv"
...
# fname_initial.h5 = "Inputs/test_2005/HFR_H_2005_Initial.h5"
fname_initial.h5 = "Inputs/test_2007/HFR_H_2007_Initial.h5"
BC.h5 = "Inputs/HFR_H_BC.h5"

# fname_head.bc.r= "results/HFR_head_BC.r"
fname_wells.r = "results/well_compiled_wl_data.r"
fname_fig.initalH_contour = "figures/initial_head.jpg"
# fname_fig.initialH_krige = "figures/initial_head_krige.jpg"
fname_fig.initialH_idw = "figures/initial_head_"
...
#               "USGS_wells", "USGS_wells_ids", "SFA_wells", "SFA_wells_ids"), file = fname_wells.r)
...
data = data.frame(x=selected.wells.unique$Easting, y=selected.wells.unique$Northing, z = selected.wells.unique$WL)
# colnames(data) = c('x','y','z')
data = data[order(data$x),]

# ##----------------------- plot well head----------------------
#         # save(data, file = "results/inital_data_coords.r")
#         
#         
#         plot(data$x, data$y, asp = 1)
#         # par(mfrow = c(2, 1))
...
#         # dev.off()
  ...
  ## ---------------use inverse distance interpolation------------------
       ...
       #  y.range <- range.ycoods  # min/max latitude of the interpolation area
        ...
        jpeg(paste(fname_fig.initialH_idw,itime, ".jpg", sep = ""), width=8,height=8,units='in',res=300,quality=100)
        # plot(data$x, data$y, col = "black", pch = 1, asp=1, xlim = c(x.range[1], x.range[2]))
        head4plot = h.initial
        ...
        points(data$x, data$y, col = "white", pch = 1, asp=1)
        polygon(river.geometry[, 1], river.geometry[, 2], border = "gray", asp=1)
...
h5write(t(h.initial),fname_initial.h5,
...
meta.data = h5read(fname_initial.h5, name="Initial_Head/Data")
h.initial
