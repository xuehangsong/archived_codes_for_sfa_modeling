...
fname_initial.h5 = "Inputs/HFR_model_250m/HFR_H_Initial2.h5"
...
fname_fig.initialH_idw = "figures/initial_head_250m.jpg"
...
# initial.time = as.POSIXct("2008-01-01 00:00:00",tz="GMT",format="%Y-%m-%d %H:%M:%S")
# times = initial.time
...
# write.csv(selected.wells.df, file = "results/data_new.csv")
...
save(selected.wells.df, file = fname.selected.wells.df) 
  ...
  if (dim(selected.wells.df)[1]>2) {
    ...
    # old_grd <- expand.grid(x = seq(from = range.xcoods[1], to = range.xcoods[2], by = idx), y = seq(from = range.ycoods[1],to = range.ycoods[2], by = idy))  # expand points to grid
...
# ##
#     idw.interp_old = idw(values=selected.wells.df[,"z"],
#                      coords = selected.wells.df[,c("x","y")],
#                      grid=old_grd,
#                      method="shepard",
#                      p=3)
#     idw.interp2 = as.numeric(unlist(idw.interp_old))
#     
#     h.initial2 = array(idw.interp2, c(length(unique(old_grd$x)), length(unique(old_grd$y)))) 
#  ##   
    ...
    itime = as.POSIXct("2007-03-28 12:00:00",tz="GMT",format="%Y-%m-%d %H:%M:%S")
...
# s1 = interp(selected.wells.df$x,selected.wells.df$y, selected.wells.df$z, duplicate = "strip", nx=100, ny=100)
# s2 = interp(data$x, data$y, data$z, duplicate = "strip", nx=100, ny=100)
# par(mfrow=c(2,1))
# image2D(s1, shade=0.2, rasterImage = F, NAcol = "gray",
#         main = c("IniH_new"), asp = 1)
# 
# image2D(s2, shade=0.2, rasterImage = F, NAcol = "gray",
#         main = c("IniH_old"), asp = 1)
...
## why tranpose? to match HDF5 format
h5write(t(h.initial2),fname_initial.h5, 
...
# 
# par(mfrow=c(3,1))
# image2D(z= old_ini, x = seq(from = range.xcoods[1], to = range.xcoods[2], by = idx), y = seq(from = range.ycoods[1],to = range.ycoods[2], by = idy), shade=0.2, rasterImage = F, NAcol = "white",
#         main = c("old_ini"), asp = 1)
# image2D(z= h.initial, x = seq(from = range.xcoods[1] + 0.5*idx, to = range.xcoods[2]-0.5*idx, by = idx), y = seq(from = range.ycoods[1] + 0.5*idy,to = range.ycoods[2], by = idy), shade=0.2, rasterImage = F, NAcol = "white",
#         main = c("new_ini"), asp = 1)
# image2D(z= h.initial2, x = seq(from = range.xcoods[1], to = range.xcoods[2], by = idx), y = seq(from = range.ycoods[1],to = range.ycoods[2], by = idy), shade=0.2, rasterImage = F, NAcol = "white",
#         main = c("old_ini2"), asp = 1)
# dev.off()
# load("results/inital_data_coords.r")
# 
