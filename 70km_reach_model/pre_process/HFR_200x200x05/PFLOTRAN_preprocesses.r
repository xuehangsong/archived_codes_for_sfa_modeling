...
fname_material_h5 = paste(input_dir, "HFR_model_", imodel, "/HFR_material_river_has_basalt.h5", sep = "")
...
output_dir = "~/Dropbox/PNNL/Projects/Reach_scale_model/Outputs/"
...
which(is.na(t.simu_wl), arr.ind = TRUE)

temp.x = seq(551700, 611500, by = 200)
temp.y = seq(104600, 164400, by = 200)
temp.coord = expand.grid(temp.x, temp.y)

temp.z = c(data.matrix(simu_wl))

temp.df = data.frame(x = temp.x, y = temp.y, z = temp.z)

temp.df = temp.df[complete.cases(temp.df), ]

temp.df = temp.df[!duplicated(temp.df), ]

temp.head = interp(x = temp.df$x, y = temp.df$y, z = temp.df$z, xo = unit_x, yo = unit_y, 
                   linear = FALSE, extrap = TRUE, duplicate = "strip", dupfun = NULL)

temp.head = bilinear.grid(model_200$x, model_200$y, model_200$z, xlim = c(551600, 611600), ylim = c(104500, 164500),
                         nx = 600, ny = 600)
