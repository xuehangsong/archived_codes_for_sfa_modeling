...
simu.ensemble = ch2o.ensemble[,1:2,]*1000*32.0
...
jpeg(paste("figures/",case.name,"s10.carbon.forward.jpg",sep=''),width=14,height=8.4,units='in',res=200,quality=100)
...
##lines(interp.time[compare.time],simu.ensemble[18,compare.time],col='orange',lwd=3)


axis(side = 2,at=seq(0,12,4),col.axis='black',col='black',line=0.5,las=2,cex.axis=1.8)
mtext(side = 2,at=6,text='CH2O (mg/L)',col='black',line=4,cex=1.8)


legend(x=start.time-0.8*3600*24,y= 23,c("Inland WL","River WL","S10 CH2O","Simulated CH2O","Best Realization"),
       cex=1.5,
       lty=c(1,1,NA,1,1),
       pch=c(NA,NA,16,NA,NA),       
       col=c("black","blue","black","green","orangered"),
       lwd=c(3,3,3,3,3),
       bty="n",horiz=TRUE)

legend(x=start.time+3600*24,y= 21,c("2-2 WL","4-9 WL","NRG WL","SWS-1 WL"),
       cex=1.5,
       lty=c(2,3,2,3),
