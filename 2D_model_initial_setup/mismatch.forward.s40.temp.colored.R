...
##load("results/s40.temp.r")

compare.time = seq(24,1081)
...
{
    obs.value = (spc.temp.value[["S40"]]-spc.temp.value[["SWS-1"]])/(spc.temp.value[["2-2"]]-spc.temp.value[["SWS-1"]])
}
obs.value = obs.value
case.name = "96"
load(paste(case.name,"/statistics/tracer.ensemble.r",sep=''))
...
lines(interp.time[compare.time],simu.ensemble[1,compare.time],col='orangered',lwd=5)
