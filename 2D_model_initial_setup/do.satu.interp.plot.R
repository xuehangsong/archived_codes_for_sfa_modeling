...
do.satu.value = list()
for (i.obs in c("2-2","S10","S40","NRG","T3"))
{
    salinity= ec2pss(spc.value[[i.obs]],do.temp.value[[i.obs]],p=0)
    do.satu.value[[i.obs]] = oxySol(do.temp.value[[i.obs]], salinity,P=NULL)
    do.satu.value[[i.obs]] = do.value[[i.obs]]/do.satu.value[[i.obs]]*100
}
...
jpeg(paste("figures/do.satu.slice.",i,".jpg",sep=''),width=14,height=8.4,units='in',res=200,quality=100)
##jpeg(paste("figures/do.satu.slice.",i,".jpg",sep=''),width=10,height=8.4,units='in',res=200,quality=100)
par(mar=c(5,5,4,5))
...
plot(interp.time,do.satu.value[["2-2"]],
     col='black',
     xlim=time.range,
     ylim=c(0.,150),
     type='l',
...
lines(interp.time,do.satu.value[["S10"]],col='green',lwd=2)
lines(interp.time,do.satu.value[["S40"]],col='red',lwd=2)
lines(interp.time,do.satu.value[["SWS-1"]],col='blue',lwd=2)
lines(interp.time,do.satu.value[["NRG"]],col='blue',lwd=2,lty=2)
...
mtext(side = 2,text='DO Saturation (%)',col='black',line=2.5)
...
#max.S40 = which(diff(sign(diff(do.satu.value[["S40"]])))==-2)+1
