...
jpeg(paste("figures/multi.level.s10.forward.jpg",sep=''),width=14,height=8.4,units='in',res=200,quality=100)
par(mar=c(5,5,4,5))
...
lines(interp.time[compare.time],simu.ensemble[1,],col='blue',lwd=2)
lines(interp.time[compare.time],simu.ensemble[2,],col='red',lwd=2)
lines(interp.time[compare.time],simu.ensemble[3,],col='green',lwd=2)
lines(interp.time[compare.time],simu.ensemble[4,],col='black',lwd=2)
lines(interp.time[compare.time],simu.ensemble[5,],col='blue',lwd=2,lty=2)
lines(interp.time[compare.time],simu.ensemble[6,],col='red',lwd=2,lty=2)
lines(interp.time[compare.time],simu.ensemble[7,],col='green',lwd=2,lty=2)
...
legend("top",c("2-3 WL","4-9 WL","SWS-1 WL","NRG WL",
               "S10 SpC",
               "upper layer","bottom layer","two layers","base"),
       cex=0.8,
       lty=c(2,1,1,2,
             NA,
             1,1,1,1,1),
