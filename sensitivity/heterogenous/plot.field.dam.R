...
combined.perm[which(material.ids==1)] = log10(hanford.perm[,19+2])[which(material.ids==1)]
combined.perm[which(material.ids==5)] = log10(alluvium.perm[,15+2])[which(material.ids==5)]
...
jpegfile=paste("./figures/agu.jpg",sep="")
jpeg(jpegfile,width = 5.4,height = 1.3,units = "in",res = 600, quality = 100)
par(mgp=c(2.,0.6,0),mar=c(2.5,3,1,1))
##jpeg(jpegfile,width=9.8,height=2.88,units='in',res=300,,quality=100)
filled.contour(x,z,array(combined.perm,c(nx,nz)),asp=1.0,color=function(x)rev(topo.colors(x)),
               ...
               plot.axes = {axis(1,at=seq(0,140,20),mgp=c(2,0.5,0),tck=-0.08,cex.axis=1)
                   ...
                   mtext('Rotated east-west direction (m)',side=1,line=1.5,cex=1)
...
## jpeg(jpegfile,width=9.8,height=2.88,units='in',res=600,,quality=100)
jpegfile=paste("./figures/agu_long.pdf",sep="")
pdf(jpegfile,width=9.8,height=2.88)
##par(mgp=c(2.,0.6,0),mar=c(2.5,3,1,1))
filled.contour(x,z,array(combined.perm,c(nx,nz)),asp=1.0,color=function(x)rev(topo.colors(x)),
               zlim=c(-15,-6),
               xlim = c(0,143.2),
               ylim = c(90,110),
               key.title = expression(paste("Perm\n (log10 m"^"2",")",sep="")),
               plot.axes = {axis(1,at=seq(0,140,20),mgp=c(2,0.5,0),tck=-0.08,cex.axis=1)
                   axis(1,at=seq(0,140,10),labels=NA,tck=-0.05)
                   axis(2,at=seq(90,110,5),mgp=c(2,0.5,0),tck=-0.06,cex.axis=1,adj=1)
                   axis(2,at=seq(90,110,5),labels=NA,tck=-0.05)
