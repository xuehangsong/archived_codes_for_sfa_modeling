...
alluvium.p = as.list(c(-2.302585,2,8,10,90+9))
...
for (isim in 1:nsim)
{
    ...
    temp[which(temp>10^hanford.threshold[2])] = 10^hanford.threshold[2]
    temp[which(temp<10^hanford.threshold[1])] = 10^hanford.threshold[1]
    hanford.perm[,isim+2] = temp

    temp = alluvium.perm[,isim+2]
    temp[which(temp>10^alluvium.threshold[2])] = 10^alluvium.threshold[2]
    temp[which(temp<10^alluvium.threshold[1])] = 10^alluvium.threshold[1]
