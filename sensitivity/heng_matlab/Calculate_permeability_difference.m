...
for i = 1:16
    for j = 1:35
        ...
        %             end
        %         end
        if strcmp(wellnamegroup_base{j}(6:end),wellnames_data{i})
            ...
            for k = 1:nsim
                fluxavc1_interp{k,i} = interp1(Time_base{k}(j,2:end),fluxavc1_base{k}(j,2:end),time_0{i}(find(time_0{i}>=1):find(time_0{i}>=1000)-1));
                ...
                distance_1t(k,i) = sum(abs(fluxavc1_interp{k,i}-Cl_revise{i}(find(time_0{i}>=1):find(time_0{i}>=1000)-1)));
