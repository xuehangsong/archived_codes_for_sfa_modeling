...
function [Time,Head_1,fluxavc1,wellname,wellnamegroup] = Read_observation_well_data_uncertainty_190_hrs(openfolder1)
...
for i = 1:inputn
    ...
    clear wellname_temp;
    for k = 1:randcpu
        if strfind(openfolder1,'BC_1_MID_1')
            openfolder = sprintf([openfolder1...
                '/pflotran1R%d-obs-%d.tec'],i,k);
        else
            openfolder = sprintf([openfolder1...
                '/pflotran2R%d-obs-%d.tec'],i,k);
        end
        if exist(openfolder,'file');
            ...
            for jj = 1:numofwells
                wellname_temp{welln-numofwells+jj} = tline(wellindex((jj-1)*6+1):wellindex((jj-1)*6+1)+10);
                if (i == 1)
                    wellname{welln-numofwells+jj} = tline(wellindex((jj-1)*6+1):wellindex((jj-1)*6+1)+10);
                end
            end
            ii = 1;
            while (ischar(tline))
