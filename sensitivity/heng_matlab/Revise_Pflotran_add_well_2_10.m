...
for j = 1:100
    C{60} = sprintf('  FILENAME ../BC_sim%d.h5',j);
    C{66} = sprintf('  FILENAME ../BC_sim%d.h5',j);
    C{72} = sprintf('  FILENAME ../BC_sim%d.h5',j);
    C{78} = sprintf('  FILENAME ../BC_sim%d.h5',j);

    fid = fopen(sprintf('C:/Users/daih524/Desktop/2015_Spring/Data_for_Test_Case/BC_Data_From_Chen/4heng/Input_deck_Add_Well_2_10/pflotran%d.in',j), 'w');
    for i = 1:numel(C)
        if C{i+1} == -1
            fprintf(fid,'%s', C{i});
            break
