    ...
    for i = 1:mx
        for j = 1:my
            for ii = 1:S
                ...
                mean_c12_M(ii,i,j) = 0;
                var_c1_M(ii,i,j) = 0;
                mean_var_c1_M(ii,i,j) = 0;
                for jj = 1:8
                    mean_c1_M(ii,i,j) = mean_c1_M(ii,i,j) + M_weight_CS*mean_c1(jj,ii,i,j);
                    mean_c12_M(ii,i,j) = mean_c12_M(ii,i,j) + M_weight_CS*(mean_c1(jj,ii,i,j)^2);
                    mean_var_c1_M(ii,i,j) = mean_var_c1_M(ii,i,j) + M_weight_CS*var_c1(jj,ii,i,j);
                end
                for jj = 9:10
                    mean_c1_M(ii,i,j) = mean_c1_M(ii,i,j) + M_weight_OK*mean_c1(jj,ii,i,j);
                    mean_c12_M(ii,i,j) = mean_c12_M(ii,i,j) + M_weight_OK*(mean_c1(jj,ii,i,j)^2);
                    mean_var_c1_M(ii,i,j) = mean_var_c1_M(ii,i,j) + M_weight_OK*var_c1(jj,ii,i,j);
                end
            ...
            Var_M_c1(i,j) = 0;
            for jj = 1:9
                mean_c1_S(i,j) = mean_c1_S(i,j) + S_weight_CS*mean_c1_M(jj,i,j);
                mean_c12_S(i,j) = mean_c12_S(i,j) + S_weight_CS*(mean_c1_M(jj,i,j)^2);
                Var_T_c1(i,j) = Var_T_c1(i,j) + S_weight_CS*(mean_var_c1_M(jj,i,j));
                Var_M_c1(i,j) = Var_M_c1(i,j) + S_weight_CS*(var_c1_M(jj,i,j));
            end
            for jj = 10:10
                mean_c1_S(i,j) = mean_c1_S(i,j) + S_weight_OK*mean_c1_M(jj,i,j);
                mean_c12_S(i,j) = mean_c12_S(i,j) + S_weight_OK*(mean_c1_M(jj,i,j)^2);
                Var_T_c1(i,j) = Var_T_c1(i,j) + S_weight_OK*(mean_var_c1_M(jj,i,j));
                Var_M_c1(i,j) = Var_M_c1(i,j) + S_weight_OK*(var_c1_M(jj,i,j));
            end
            ...
            S_S_c1_new(i,j,tt) = Var_S_c1(i,j)/(Var_S_c1(i,j)+Var_M_c1(i,j)+Var_T_c1(i,j));
            S_M_c1_new(i,j,tt) = Var_M_c1(i,j)/(Var_S_c1(i,j)+Var_M_c1(i,j)+Var_T_c1(i,j));
            S_T_c1_new(i,j,tt) = Var_T_c1(i,j)/(Var_S_c1(i,j)+Var_M_c1(i,j)+Var_T_c1(i,j));
