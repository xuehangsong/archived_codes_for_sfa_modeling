...
for i = 1:16
    for j = 1:82
        wellnamegroup_76_new{j} = strrep(wellnamegroup_76_new{j},'2-0','2-');
        ...
        if strcmp(wellnamegroup_76_new{j}(6:end),wellnames_data{i})
            subplot(4,4,i);
            hold all;
            %plot(time_0{i},max((Br0{i}-mean(Br01{i}(end-10:end)))/100,0),'k*');
            plot(Time_76_new{1}(1,:),fluxavc1_76_new{1}(j,:)/0.001,'-','LineWidth',2);
            plot(Time_76_old{1}(1,:),fluxavc1_76_old{1}(j9,:)/0.001,'-','LineWidth',2);
