...
#update mud layer
hori.elevation = 99.5 ##################################105
hori.length = 0.05 #7
second.depth = 6.5 #1.5
third.depth = 2
...
obs.dz = rep(0.2,100)
obs.depth = cumsum(obs.dz)
obs.well.x = c()
obs.well.z = c()
for (rivel.l in seq(103,107,0.5))
{
    obs.well.x.temp = x[river.bank.x[which.min(abs(z[river.bank.z]-rivel.l))]]
    obs.well.z.temp = z[river.bank.z[which.min(abs(z[river.bank.z]-rivel.l))]]-obs.depth
    obs.well.z.temp = obs.well.z.temp[obs.well.z.temp+2>z[hanford.ringold.boundary][obs.well.x.temp]]
    obs.well.x.temp = rep(obs.well.x.temp,length(obs.well.z.temp))
    obs.well.x = c(obs.well.x,obs.well.x.temp)
    obs.well.z = c(obs.well.z,obs.well.z.temp)    
}
    


save(list = c("obs.well.depth","obs.well.x","obs.well.z"),file='results/obs.well.loc.r')
write.table(cbind(x[river.bank.x],z[river.bank.z],z[river.bank.z]-mudlayer.depth),file="alluvium.shape.txt",row.names=FALSE,col.names=FALSE)
