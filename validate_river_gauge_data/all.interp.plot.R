...
start.time = as.POSIXct("2016-03-26 00:00:00",tz="GMT",format="%Y-%m-%d %H:%M:%S")
end.time = as.POSIXct("2016-05-10 00:00:00",tz="GMT",format="%Y-%m-%d %H:%M:%S")
...
axis(side = 2,at=c(104,105,106,107),col.axis='black',col='black',las=2,line=0,cex.axis=1.7)
...
lines(interp.time,spc.value[["RG3"]]/1000,col='orange',lwd=2)
...
lines(interp.time,spc.temp.value[["RG3"]],col='orange',lwd=2)
...
legend(mean(c(start.time,end.time))-24*23*3600,59,
       c("2-3","SWS-1","RG3","S10","S40"),
       cex=1.8,
       lty=c(1,1,1,1,1),
