...
for (ipath in 1:npath)
{
	...
	sim_data = array(0,c(nfields,sum(nt_obs))) # ensemble of simulated data
        ...
        write.table(obs_ensemble,file=paste(main_path,paths[ipath],'/obs_ensemble_',min_time,'to',max_time_upper,'h_Err',err_cv,'_300.txt',sep=''),row.names=F,col.names=F)
        ...
        write.table(cor_yd_perm,file=paste(main_path,paths[ipath],'/cor_yd_perm_',min_time,'to',max_time_upper,'h_Err',err_cv,'_300.txt',sep=''),row.names=F,col.names=F)
        rm(cor_yd_perm)
	...
	cov_dd[which(abs(cor_dd)<0.05*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter',new_iter,'_300.txt',sep=''),row.names=F,col.names=F)
}
