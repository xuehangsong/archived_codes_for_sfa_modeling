#This file is used for smoother updating of perm field with localization and log-transformed concentration
...
perm_par = read.table(paste(prior_path,'/','logK_3D_Kriged_Samples_600.txt',sep=''))
new_iter = 1
...
for (ivar in 1:nwell)
{
       ...
       trueBTC[which(trueBTC<0)] = 0.000001
...
for (ipath in 1:npath)
{
	...
	sim_data = array(0,c(nfields,sum(nt_obs))) # ensemble of simulated data
        ...
        obs_ensemble[which(obs_ensemble<0)] = 0.000001  #remove negative concentrations        
        #save the ensembles for observations
        write.table(obs_ensemble,file=paste(main_path,paths[ipath],'/obs_ensemble_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter1.txt',sep=''),row.names=F,col.names=F)

        #load the material ids for each permeability
        material_ids = read.table('material_ids.txt')
	#load the indicators whether localization should be applied, 1 means exclusion in update
        loc_ids = read.table('loc_id.txt')
	...
	#make the correlation of cells two far away 0
        cor_yd_perm[which(loc_ids == 1),] = 0
        cov_yd_perm[which(loc_ids == 1),] = 0
        ...
        write.table(cor_yd_perm,file=paste(main_path,paths[ipath],'/cor_log_yd_perm_',min_time,'to',max_time_upper,'h_Err',err_cv,'_localized.txt',sep=''),row.names=F,col.names=F)
        rm(cor_yd_perm)
        cov_dd = cov(log(sim_data),log(sim_data))
        cor_dd = cor(log(sim_data),log(sim_data))
	cov_dd[which(abs(cor_dd)<0.05*max(abs(cor_dd)))] = 0.0
        ...
        for (ipar in 1:nfields)
                new_perm_par[ipar,] = perm_par[ipar,] + gain_perm %*% (matrix(log(obs_ensemble[ipar,])-log(sim_data[ipar,]),sum(nt_obs),1))
        ...
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Localized_Transformed_Iter',new_iter,'.txt',sep=''),row.names=F,col.names=F)
}
