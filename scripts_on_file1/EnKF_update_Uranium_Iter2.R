...
paths = c('Uranium_Mar2011_EnKF_0to300h_Iter1/')
prior_path = 'Uranium_Mar2011_EnKF_Prior'
npath = length(paths)
fields = c(1:200)
par_idx = c(1:200)
nfields = length(fields)     # number of random fields
#load the ensemble of parameters
prior_par = read.table(paste(prior_path,'/','InitialU_Post_Samples_0to300h_Err10_Iter1.txt',sep=''))
...
for (ipath in 1:npath)
{
	...
	cov_dd[which(abs(cor_dd)<0.01*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(post_par),file=paste(main_path,paths[ipath],'/InitialU_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter2.txt',sep=''),row.names=F,col.names=F)
}
