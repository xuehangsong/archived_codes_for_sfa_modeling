#for updating parameters using EnKF, cells above 108m excluded in updating
...
for (ipath in 1:npath)
{
	...
	sim_data = array(0,c(nfields,sum(nt_obs))) # ensemble of simulated data
        ...
        {
                meas_err[ii,ii] = (max(BTC_obs[ii] * err_cv_val,0.01*err_cv_val))^2
        ...
        high_vz_ids = read.table('high_VZ_id.txt')
        ...
        #make the correlations between simulated data and cells above 108m 0
        cor_yd_perm[which(high_vz_ids == 1),] = 0
        cov_yd_perm[which(high_vz_ids == 1),] = 0
        ...
        write.table(cor_yd_perm,file=paste(main_path,paths[ipath],'/No_VZ_cor_yd_perm_',min_time,'to',max_time_upper,'h_Err',err_cv,'.txt',sep=''),row.names=F,col.names=F)
        rm(cor_yd_perm)
	...
	cov_dd[which(abs(cor_dd)<0.01*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/No_VZ_logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter',new_iter,'.txt',sep=''),row.names=F,col.names=F)
}
