...
paths = c('Uranium_Mar2011_Iter3')
vlines = c(250,300,350)
...
for (ipath in 1:npath)
{
 	...
 	for (ivar in 1:nwell)
	{
		...
		if(true_exist)
		{
				...
				trueBTC = truedata[,2]/NormConc_True
		}
		...
		if(nfields > 1)
			plot_y = data0[,1]
		plot(fixedt,plot_y, main=paste(plot_title,well_name,sep=''), xlab = b[1], ylab = y_label,xlim=c(1,1000), ylim = c(0, ymax), type = "l",lwd = linwd1, col = line_col,lty = 'solid',bty="n",cex.lab=1.7,cex.axis=1.7,cex.main=1.8) 
		if(nfields > 1)
		{
