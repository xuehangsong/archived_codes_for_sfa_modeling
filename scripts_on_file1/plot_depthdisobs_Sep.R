...
paths = c('A_MR_0','A_EQ_0','B_EQ_0','B_MR_0')
...
for (ipath in 1:npath)
{
	...
	for (iw in 1:nwell)
	{
		...
		for (i in 1:nplots_tracer)
		{
			...
			plot(fixedt,Tracer.mean[ilocs], main=paste(paths[ipath],': ',Tracer_titles[ids_tracer[i]],sep=''), xlab = '', ylab = '',xlim=c(-200,750), ylim = c(ymin, ymax), type = "l",lwd = linwd1, col = 'red',lty = 'solid') 
		...
		#legend("topleft",c("Mean","95% CI",'True'), lty=c('solid','dashed','solid'),col=c('red','blue','black'),lwd=c(linwd1,linwd1,linwd1),pch=c(-1,-1,1), bty='n')
			...
			jpeg(paste(paths[ipath],'_',prefix_file,'_CI_Well', wells[iw],UO2_titles[ids_UO2[i]], plotfile, sep = ''), width = pwidth, height = pheight,units="in",res=150,quality=100)
                        ...
                        plot(fixedt,UO2.mean[ilocs], main=paste(paths[ipath],': ',UO2_titles[ids_UO2[i]],sep=''), xlab = '', ylab = '',xlim=c(-200,750), ylim = c(ymin, ymax), type = "l",lwd = linwd1, col = 'red',lty = 'solid') 
