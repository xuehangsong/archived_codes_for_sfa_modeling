...
#paths = c('Tracer_Mar2011_UK3_SmootherD_Err30_Iter3')
paths = c('Tracer_Jun2010')
npath = length(paths)
time_start = 1840.0 #Mar2011 test
fields =  c(184) 
nfields = length(fields)     # number of random fields
testvariable = 'Tracer2_37'
...
for (ipath in 1:npath)
{
 	...
 	for (ivar in 1:nwell)
	{
		...
		data0 = as.matrix(data0)
		ymax = max(data0,na.rm=T)
		ymin = min(data0,na.rm=T)
		if(true_exist)
		{
		...
		plot(fixedt,data0[,1], main=paste(plot_title,well_name,sep=''), xlab = b[1], ylab = y_label,xlim=c(0,800), ylim = c(0, ymax), type = "l",lwd = linwd1, col = 'darkred',lty = 'solid',bty="n",cex.lab=1.7,cex.axis=1.7,cex.main=1.8) 
		if(nfields>1)
		{
