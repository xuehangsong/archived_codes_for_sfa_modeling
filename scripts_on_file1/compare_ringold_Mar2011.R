...
main_path = '/files1/scratch/chenxy/Mar2011/'
comp_paths = c('MeanField/IFRC2/120x120x30','MeanField_noRingold/IFRC2/120x120x30')
legend_lab = c('Ringold','NoRingold')
paths = c('IFRC2')
#paths = c('A','B')
npath = length(paths)
...
{
	path_prefix1 = paste(main_path,comp_paths[1],'/',sep='')
	a = readLines(paste(path_prefix1, prefix_file,'R1',ext_file,sep=''),n=1)
	b = unlist(strsplit(a,','))

	path_prefix2 = paste(main_path,comp_paths[2],'/',sep='')
	...
	for(ifield in seq(1,nfields))
	{
		...
		jpeg(paste(main_path,plotfile_prefix,paths[ipath],'_',ifield, plotfile, sep = ''), width = pwidth, height = pheight,units="in",res=150,quality=100)
		par(mfrow=c(6,7))
    		for (ivar in 1:nvar)
    		{
			...
			if(ivar == 1)
				legend('topright',legend_lab,lty=c('solid','solid'),col=c('red','blue'),lwd=c(linwd1,linwd1),bty='n')
