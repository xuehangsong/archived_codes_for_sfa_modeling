...
paths = c('Tracer_Mar2011_UK_EnKF_Batch_100to200h_Err10_Iter2_2/')
npath = length(paths)
fields = c(1:200)
par_idx = c(1:200)
nfields = length(fields)     # number of random fields
#load the prior ensemble of parameters
perm_par = read.table('Tracer_Mar2011_UK_EnKF_Batch_500h_Iter1/logK_Post_Samples_500h_Err10_Iter2.txt')
...
err_cv_val=0.10
min_time = 500
data_ext = paste('upto',max_time_upper,'hr.Rdata',sep="")
...
for (ipath in 1:npath)
{
	...
	write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter3.txt',sep=''),row.names=F,col.names=F)	
}
