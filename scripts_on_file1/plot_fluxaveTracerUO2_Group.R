...
true_col = c(3,2)
NormConc_True = c(100.0, 1.0) #for Mar2011 test
y_label = c('Tracer C/C0','U(VI) [ug/L]')
#y_convert = c(1000.0/5.9234,238.0*1000000) #for Mar2011 test
#y_convert = c(1000.0/7.5,238.0*1000000) #for test on 10 Oct 2011
y_convert = c(1000.0/2.8169,238.0*1000000) #for test on 19 Oct 2011

#time_start = 191.25 #Mar2011 test
#time_start = 1.0 # for premodeling Oct'11 test
time_start = 0.5
...
paths = c('Corner_19Oct_HigherPorosity')
npath = length(paths)
fields =  c(1)
...
pwidth =25 
pheight = 17
plotfile = '_MeanField_Selected.jpg'
...
#wells_plot = c('2-07','2-08','2-09','2-11','2-12','2-14','2-15','2-26','2-28','2-34')
plot_nrow = 6
plot_ncol = 7
...
for (ipath in 1:npath)
{
	...
	for(ifield in fields)
	{
		...
		data_1 = data0[,varcols_1] * y_convert[1]
		...
		#par(mfrow=c(5,8),mar=c(4,4,4,4))
		par(mfrow=c(plot_nrow,plot_ncol),mar=c(2,3.5,2,3.5),cex = 1.2)
		for (ivar in 1:nwell)
		{
			...
			plot(t0,data_1[,ivar],main=well_name,xlab="",ylab="",xlim=c(time_low,time_upper),ylim=c(0,1),type='l',col='black',lty='solid',lwd=linwd1,axes=FALSE,cex.lab=1.2)
			if(plot_true)
			...
			plot(t0,data_2[,ivar], xlab="", ylab="",xlim=c(time_low,time_upper),ylim=c(0,max(data_2[,ivar],na.rm=T)+1),axes=FALSE, type='l',col='gold4',lty='solid',cex.lab=1.2)	
			if(plot_true)
				lines(true_t, trueBTC_2, lwd=linwd1,type="b",lty='solid', pch=2,col="gold4")		
## a little farther out (line=4) to make room for labels
			mtext(y_label[2],side=4,col="gold4",line=2.0) 
			axis(4, col="gold4",col.axis="gold4",las=1)
			grid(26,NA)	
## Draw the time axis
			axis(1,at=seq(time_low,time_upper,12))
		...
		if(!plot_true)	
			legend("topleft",c("Tracer_sim",'UO2_sim'), lty=c('solid','solid'),col=c('black','gold4'),lwd=c(linwd1,linwd1), cex=1.5, bty='n')
