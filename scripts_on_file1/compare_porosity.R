...
paths = c('A_MR_0','A_MR_0_porosity')
npath = length(paths)
...
for (itest in 1:ntest)
{
	...
	for(ifield in fields)
	{
		...
		jpeg(paste(main_path,'Comp_Porosity_A_MR_0_',testvariable, 'R',ifield, plotfile, sep = ''), width = pwidth, height = pheight,units="in",res=150,quality=100)
		par(mfrow=c(6,6))
		for (ivar in 1:nvar)
        {
		...
		legend("topleft",c('Uniform porosity','Hetero porosity','True'), lty=c('solid','solid','solid'),col=c('red','blue','black'),lwd=c(linwd1,linwd1,linwd1),pch=c(-1,-1,1), cex=2.5, bty='n')
		dev.off()
	}
}
