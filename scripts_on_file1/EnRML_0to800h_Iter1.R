...
wells_data = array(0,c(0,1)) # wells that have data within the time window
err_cv='10'
err_cv_val=0.10
...
for (ivar in 1:nwell)
{
       ...
       if(length(grep('2-34',well_name)))# excluding the injection for well 2-34 
...
for (ipath in 1:npath)
{
	...
	sim_data = array(0,c(nfields,sum(nt_obs))) # ensemble of simulated data
        ...
        for (ipar in 1:nfields)
                new_perm_par[ipar,] = perm_par[ipar,] + beta * gain_perm %*% (matrix(obs_ensemble[ipar,]-sim_data[ipar,],sum(nt_obs),1))
        new_perm_par[which(new_perm_par<=-11.0)] = -11.0 + rnorm(length(which(new_perm_par<=-11.0)),0,0.1)
        new_perm_par[which(new_perm_par>=3.0)] = 3.0 + rnorm(length(which(new_perm_par>=3.0)),0,0.1)

        #save the updated parameter sets to text files
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_EnRML_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter',new_iter,'_set1.txt',sep=''),row.names=F,col.names=F)
}
