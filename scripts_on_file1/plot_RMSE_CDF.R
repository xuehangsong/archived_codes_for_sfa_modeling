...
npath = length(paths)
plotfile_name = 'Comp_RMSE_PDF.jpg'
legends = c('Prior','Batch_Iter2','4_Seg','16_Seg')
plot_legend = T
plot_cols = c('black','darkgreen','darkred','goldenrod')
pwidth = 26
pheight = 17
plot_nrow = 5
plot_ncol = 7
...
for (iw in 1:nwell)
{
	...
	for (ipath in 1:npath)
	{
		...
		xmax = max(max(dens$x,na.rm=T),xmax)
	...
	#plot
	plot(density(RMSE_1[,iw],bw='nrd'), main = wells[iw],xlim = c(xmin, xmax),ylim = c(ymin,ymax),xlab = 'RMSE of C/C0',ylab = 'Density',axes=F,lty=1,col=plot_cols[1])
