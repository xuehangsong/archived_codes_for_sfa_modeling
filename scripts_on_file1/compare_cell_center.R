...
paths = c('A_MR_0','A_MR_0_cell_center')
npath = length(paths)
...
for (itest in 1:ntest)
{
	...
	for(ifield in fields)
	{
		...
		jpeg(paste(main_path,'Comp_Cell_Center_A_MR_0_',testvariable, 'R',ifield, plotfile, sep = ''), width = pwidth, height = pheight,units="in",res=150,quality=100)
		par(mfrow=c(6,6))
		for (ivar in 1:nvar)
        {
		...
		#read in true data if it exists         
			...
			plot(t1,BTC1, main=paste('A_MR_0: ',well_name,sep=''), xlab = b[1], ylab = y_label,xlim=c(-200,750), ylim = c(ymin, ymax), type = "l",lwd = linwd1, col = 'red',lty = 'solid',bty="n") 
		...
		legend("topleft",c('Coordinate_based','Cell_Center','True'), lty=c('solid','solid','solid'),col=c('red','blue','black'),lwd=c(linwd1,linwd1,linwd1),pch=c(-1,-1,1), cex=2.5, bty='n')
		dev.off()
	}
}
