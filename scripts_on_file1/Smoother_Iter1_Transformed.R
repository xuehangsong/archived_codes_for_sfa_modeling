...
for (ivar in 1:nwell)
{
       ...
       if(length(grep('3-24',well_name)))#well 3-24 may have intra-wellbore flow after 500 hours
		id_t = which(true_t<=min(max_time_upper,500) & true_t>=min_time)
       true_t = true_t[id_t]
...
for (ipath in 1:npath)
{
	...
	meas_err = matrix(0,sum(nt_obs),sum(nt_obs))
        ...
        obs_ensemble = read.table(paste(main_path,paths[ipath],'/obs_ensemble_',min_time,'to',max_time_upper,'h_Err',err_cv,'.txt',sep=''))
	obs_ensemble = as.matrix(obs_ensemble)
	obs_ensemble[which(obs_ensemble <= 0.0)] = 0.000001
        ...
        #set covarience to be 0 if its absolute value is less than 0.01
        cov_yd_perm[which(abs(cor_yd_perm)<0.001)] = 0.0
        write.table(cor_yd_perm,file=paste(main_path,paths[ipath],'/cor_log_yd_perm_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter',new_iter,'.txt',sep=''),row.names=F,col.names=F)
        rm(cor_yd_perm)
        cov_dd = cov(log(sim_data),log(sim_data))
        cor_dd = cor(log(sim_data),log(sim_data))
	cov_dd[which(abs(cor_dd)<0.001)] = 0.0
        ...
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Transformed_Iter',new_iter,'.txt',sep=''),row.names=F,col.names=F)
}
