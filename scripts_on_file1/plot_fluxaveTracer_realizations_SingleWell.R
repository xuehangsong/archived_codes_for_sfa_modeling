...
paths = c('Tracer_Mar2011_UK_KrigeFields')
npath = length(paths)
fields =  c(1,5,10)
...
for (ipath in 1:npath)
{
 	...
 	for (ivar in 1:nwell)
	{
		...
		jpeg(paste(main_path,paths[ipath],'_Realizations_',well_name, plotfile, sep = ''), width = pwidth, height = pheight,units="in",res=200,quality=100)
		par(mfrow=c(plot_nrow,plot_ncol),mar=c(4,3,3,3))
		...
		for (irel in 1:nfields){
			plot(fixedt,data0[,irel], main=paste(plot_title,well_name,sep=''), xlab = b[1], ylab = y_label,xlim=c(0,1200), ylim = c(0, ymax), type = "l",lwd = linwd1, col = 'darkred',lty = 'solid',bty="n",cex.lab=1.7,cex.axis=1.7,cex.main=1.8) 
