...
paths = c('Tracer_Mar2011_UK_EnKF_Batch_500h_Iter1')
npath = length(paths)
Mean_path = paste(main_path,paths[1],'/MeanField',sep='')
...
#pwidth =15  
#pheight = 10
#plotfile = '_Mar2011_selected.jpg'
plotfile = '.jpg'
...
#wells_plot = c('2-34','2-07','2-09','2-11','2-26','2-27','2-28')
...
for (ipath in 1:npath)
{
 	...
 	for (ivar in 1:nwell)
	{
		...
		if(true_exist)
		{
			...
			true_t = true_t[which(true_t<=time_upper)]
			trueBTC = trueBTC[which(true_t<=time_upper)]
		}
		...
		plot(fixedt,data0.mean, main=well_name, xlab = b[1], ylab = y_label,xlim=c(0,time_upper), ylim = c(ymin, ymax), type = "l",lwd = linwd1, col = 'red',lty = 'solid',cex.lab=1.5,cex.axis=1.5,bty="n") 
