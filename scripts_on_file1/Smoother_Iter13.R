...
paths = c('Tracer_Mar2011_UK3_Inj5_600to700h_Err20_Iter12/')
prior_path = 'Tracer_Mar2011_UK3_Inj5_500to600h_Err20_Iter11'
...
perm_par = read.table(paste(prior_path,'/','logK_Post_Samples_600to700h_Err20_Iter12.txt',sep=''))
new_iter = 13
...
'3-29','3-30','3-31','3-32','3-35','2-37','3-24','3-27')

max_time_upper = 750.0
min_time = 700.0
nwell = length(wells)
...
for (ipath in 1:npath)
{
	...
	sim_data = array(0,c(nfields,sum(nt_obs))) # ensemble of simulated data
        ...
        write.table(obs_ensemble,file=paste(main_path,paths[ipath],'/obs_ensemble_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter',new_iter,'.txt',sep=''),row.names=F,col.names=F)
        ...
        cov_yd_perm[which(abs(cor_yd_perm)<0.01)] = 0.0
	...
	cov_dd[which(abs(cor_dd)<0.01)] = 0.0
