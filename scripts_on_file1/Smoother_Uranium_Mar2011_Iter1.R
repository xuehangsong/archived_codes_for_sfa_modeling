...
paths = c('Uranium_Mar2011_Prior/')
npath = length(paths)
fields = c(1:200)
par_idx = c(1:200)
nfields = length(fields)     # number of random fields
#load the ensemble of parameters
prior_par = read.table('InitialU_Prior_Samples.txt')
...
for (ipath in 1:npath)
{
	...
	cov_dd[which(abs(cor_dd)<0.05*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(post_par),file=paste(main_path,paths[ipath],'/InitialU_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter1.txt',sep=''),row.names=F,col.names=F)
}
