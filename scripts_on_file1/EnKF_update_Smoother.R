...
perm_par = read.table('Field_Generation/logK_3D_Kriged_Samples.txt')
...
for (ipath in 1:npath)
{
	...
	cov_dd[which(abs(cor_dd)<0.01*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter1.txt',sep=''),row.names=F,col.names=F)
}
