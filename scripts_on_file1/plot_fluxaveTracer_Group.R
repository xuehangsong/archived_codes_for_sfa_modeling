...
testvariable = 'Tracer'
#Mean_path = paste(main_path,'MeanField/IFRC2/120x120x30',sep='')
Mean_path = '' 
BC = 'Kriging'
DataSet = 'Mar2011TT'
paths = c('Tracer_Mar2011_UK3_Iter17')
npath = length(paths)
fields =  seq(1,300)
...
for (ipath in 1:npath)
{
	...
	#plot
	jpeg(paste(main_path,prefix_file,'_CI_',testvariable, plotfile, sep = ''), width = pwidth, height = pheight,units="in",res=150,quality=100)
	par(mfrow=c(5,8))
 	for (ivar in 1:nwell)
	{
