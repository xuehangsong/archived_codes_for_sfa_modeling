...
main_path = 'Mar2009/'
grid_res = '60x60x30'
paths = c('IFRC2_steady_state','IFRC2_recharge')
...
for (ivar in 1:nvar)
	...
	for(ifield in fields)
	{
		...
		jpeg(paste(main_path, 'Comp_Recharge_IFRC2_',grid_res,'_R',ifield, plotfile, sep = ''), width = pwidth, height = pheight,units="in",res=150,quality=100)
		par(mfrow=c(6,6))
		for (ivar in 1:nvar)
		{
			...
			plot(t1,data1[,(ivar+1)], main=paste('IFRC2: ',well_name,sep=''), xlab = b[1], ylab = y_label,xlim=c(0,300), ylim = c(ymin, ymax), type = "l",lwd = linwd1, col = 'red',lty = 'solid') 
			...
			if(ivar == 1)
			legend('topright',c('0mm','55mm'),lty=c('solid','solid'),col=c('red','blue'),lwd=c(linwd1,linwd1*1.5),bty='n')
