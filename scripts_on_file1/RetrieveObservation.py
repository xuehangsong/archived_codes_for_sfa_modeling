...
fields = range(1,101)
#fields = [8, 135, 285, 310,329,474]
#number of partitions per field
n_part = 1024
#the paths to all the files
#paths = ['/tmp/work/chenxy/PflotranRun/480x480x60/Tracer_Mar2011_MeanField']
paths = ['/tmp/work/chenxy/PflotranRun/120x120x30/IFRC2_EQ_0_Mar2011']
...
for ipath in range(len(paths)):
    ...
    #for multiple fields
        ...
        for i in range(n_part):
            name_pre = 'observationR%d' % fields[0]
            name_end = '-%d.tec' % i
            name = name_pre + name_end
            filename = []
            filename.append(paths[ipath])
            filename.append(name)
            filename = '/'.join(filename)
    ...
    for ifield in range(n_file/n_parts):
        ...
        for iheader in range(len(headers)):
            ...
            #print header
        #print headers
        if ifield == 0: # only read variable names and locations from headers for the first realization
