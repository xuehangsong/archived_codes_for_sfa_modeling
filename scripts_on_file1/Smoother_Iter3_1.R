...
paths = c('Tracer_Mar2011_UK3_Inj5_Iter2/')
...
perm_par = read.table(paste(prior_path,'/','logK_Post_Samples_0to800h_Err20_Iter2.txt',sep=''))
new_iter = 3
...
for (ipath in 1:npath)
{
	...
	sim_data = array(0,c(nfields,sum(nt_obs))) # ensemble of simulated data
        ...
        cov_yd_perm[which(abs(cor_yd_perm)<0.1)] = 0.0
	...
	cov_dd[which(abs(cor_dd)<0.1)] = 0.0
