...
wells_plot = c('2-34','2-26','2-28','2-14')
...
for (ipath in 1:npath)
{
 	...
 	for (ivar in 1:nwell)
	{
		...
		ymax = 1.0
		plot(fixedt,data0[,1], main=paste(plot_title,BC,well_name,sep=''), xlab = b[1], ylab = y_label,xlim=c(0,1000), ylim = c(0, ymax), type = "l",lwd = linwd1, col = 'chocolate',lty = 'solid',bty="n",cex.lab=1.7,cex.axis=1.7,cex.main=1.8) 
		for (irel in 2:nfields)
			lines(fixedt,data0[,irel],lwd=linwd1,col='chocolate',lty='solid')
           
		points(true_t,trueBTC,col='black',type = 'b',lty='solid',lwd=linwd1,pch=1)
	}
	if(plot_legend)
		#plot(c(1,2),c(2,2),ylim=c(0,1),xlab="",ylab="",bty="n",axes = F)	
		legend("topright",c('Simulated','Observed'), lty=c('solid','solid'),col=c('chocolate','black'),lwd=c(linwd1,linwd1),pch=c(-1,1), cex=1.7, bty='n')
	dev.off() 	
}
