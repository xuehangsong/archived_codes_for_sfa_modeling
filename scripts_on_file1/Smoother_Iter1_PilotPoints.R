...
paths = c('Tracer_Mar2011_UK3_KrigeFields_Inj2')
npath = length(paths)
fields = c(1:600)
nfields = length(fields)     # number of random fields
#load the ensemble of parameters
par = read.table('Field_Generation/input_files_anchor/input_par_newEBF2.txt')
par = par[seq(1,30000,50),6:520] #600 parameter sets were used
...
'2-22','2-23','2-24','2-26','2-27','2-28','2-29','2-30','2-31','3-23','3-25','3-27','3-28',
...
for (ipath in 1:npath)
{
	...
	obs_ensemble[which(obs_ensemble>1.0)] = 1.0 
	...
	write.table(obs_ensemble,file=paste(main_path,paths[ipath],'/obs_ensemble_',min_time,'to',max_time_upper,'h_Err',err_cv,'.txt',sep=''),row.names=F,col.names=F)
        ...
        cov_yd_perm = cov(par,sim_data)
        cor_yd_perm = cor(par,sim_data)
        cov_yd_perm[which(abs(cor_yd_perm)<0.01*max(abs(cor_yd_perm)))] = 0.0
        write.table(cor_yd_perm,file=paste(main_path,paths[ipath],'/cor_yd_par_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter1.txt',sep=''),row.names=F,col.names=F)
        rm(cor_yd_perm)
	...
	{
                new_perm_par[ipar,] = par[ipar,] + gain_perm %*% (matrix(obs_ensemble[ipar,]-sim_data[ipar,],sum(nt_obs),1))
        	new_perm_par[ipar,which(new_perm_par[ipar,]<=-11.0)] = -11.0 + rnorm(length(which(new_perm_par[ipar,]<=-11.0)),0,0.1)
        	new_perm_par[ipar,which(new_perm_par[ipar,]> 1.0)] = 1.0 + rnorm(length(which(new_perm_par[ipar,]>1.0)),0,0.1)
	}
	#save the updated parameter sets to text files
	write.table(new_perm_par,file=paste(main_path,paths[ipath],'/input_par_Iter1.txt',sep=''),row.names=F,col.names=F)	
}
