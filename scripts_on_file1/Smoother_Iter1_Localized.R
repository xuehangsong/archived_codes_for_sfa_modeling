...
for (ipath in 1:npath)
{
	...
	#make the correlation of cells two far away 0
        ...
        write.table(cor_yd_perm,file=paste(main_path,paths[ipath],'/cor_yd_perm_',min_time,'to',max_time_upper,'h_Err',err_cv,'_localized.txt',sep=''),row.names=F,col.names=F)
        rm(cor_yd_perm)
	...
	cov_dd[which(abs(cor_dd)<0.05*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Localized_Iter',new_iter,'.txt',sep=''),row.names=F,col.names=F)
}
