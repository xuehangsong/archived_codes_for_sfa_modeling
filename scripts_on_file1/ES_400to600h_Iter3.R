...
paths = c('Tracer_Mar2011_200to400h_Iter2/')       
prior_path = 'Tracer_Mar2011_0to200h_Iter1'
...
perm_par = read.table(paste(prior_path,'/','logK_Post_Samples_200to400h_Err10_Iter2_set1.txt',sep=''))
new_iter = 3
...
for (ipath in 1:npath)
{
	...
	#cov_dd[which(abs(cor_dd)<0.05*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter',new_iter,'_set1.txt',sep=''),row.names=F,col.names=F)
}
