...
paths = c(paste(BC,'_MR_0',sep=''),'UK_MR_0')
npath = length(paths)
fields = seq(1,500)
...
for (ipath in 1:npath)
{
	...
	meanfile = paste(path_prefix, 'MeanField/',prefix_file,'R1',ext_file,sep='')
        ...
        eval(parse(text=string))
	 # match the column names in mean field and all realizations
        col_id = numeric(nvar)
        for (ivar in 1:nvar)
                col_id[ivar] = match(varnames[ivar],varnames_mean)
        ...
        for (ivar in 1:nvar)
        {
                ids = which(is.na(data0[,(col_id[ivar]+1)]) == 0) #the points used for interpolation
                interp = approx(t0,data0[ids,(col_id[ivar]+1)],xout = fixedt,rule = 2)$y
                #locations of the data will be stored
...
#read in true data if it exists	
	...
	for (ipath in 1:npath)
	{
		#find the corresponding variable in data
		string = paste('irow=match(varnames[ivar],varnames',ipath,')',sep='')
		eval(parse(text=string))	
		string = paste('fixedt=fixedt',ipath,sep='')
		eval(parse(text=string))	
		nt = length(fixedt)
		locs = ((irow - 1) * nt + 1) : (irow * nt)
	...
	for (ipath in 1:npath)
	{
		...
		data0.975=apply(data0,1,quantile,0.975)
		#mean data
                string = paste('meandata=meandata',ipath,'[locs,1]',sep='')
                eval(parse(text=string))
