...
paths = c('Uranium_Mar2011_Iter1/')
prior_path = 'Uranium_Mar2011_Prior/'
new_iter = 2
npath = length(paths)
fields = c(1:200)
par_idx = c(1:200)
...
prior_par = read.table(paste(main_path,prior_path,'InitialU_Post_Samples_0to50h_Err10_Iter1.txt',sep=''))
...
wells = c('2-07','2-08','2-09','2-11','2-12','2-13','2-14','2-15','2-16','2-17','2-18','2-19',
'2-26','2-28')
...
for (ipath in 1:npath)
{
	...
	material_ids = material_ids[1:par_dim,1] # only extract the ids relevant to parameters
        ...
        cov_yd_par[which(abs(cor_yd_par)<0.05*max(abs(cor_yd_par)))] = 0.0
        rm(cor_yd_par)
	# for those cells excluded for updating, use updated parameters to krige
	...
	#cov_dd[which(abs(cor_dd)<0.05*max(abs(cor_dd)))] = 0.0
        ...
        write.table(t(post_par),file=paste(main_path,paths[ipath],'/InitialU_Post_Samples_',min_time,'to',max_time_upper,'h_Err',err_cv,'_Iter',new_iter,'.txt',sep=''),row.names=F,col.names=F)
}
