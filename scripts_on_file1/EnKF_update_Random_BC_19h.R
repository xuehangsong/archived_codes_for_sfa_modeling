...
perm_par = read.table('Field_Generation/par3d_2012/logK_3D_CondSim_Samples.txt')
...
i_BC_start = 1
i_BC_end = 20
times_BC = 0:19 #corresponding beginning time for each boundary condition
...
for (ipath in 1:npath)
{
	...
	write.table(data.frame(wells_obs,t_obs,BTC_obs), file=paste(path_prefix,'OBS_points_upto',max_time_upper,'hr.txt',sep=""),quote=F,row.names=F)
	...
	write.table(obs_ensemble,file=paste(main_path,paths[ipath],'/obs_ensemble_',max_time_upper,'h.txt',sep=''),row.names=F,col.names=F)	
	...
	write.table(cor_yd_perm,file=paste(main_path,paths[ipath],'/cor_yd_perm_',max_time_upper,'h.txt',sep=''),row.names=F,col.names=F)	
	rm(cor_yd_perm)
	...
	write.table(t(new_perm_par),file=paste(main_path,paths[ipath],'/logK_Post_Samples_',max_time_upper,'h.txt',sep=''),row.names=F,col.names=F)
	rm(new_perm_par)
...
#set covarience to be 0 if its absolute value is less than 0.05
	...
	write.table(cor_yd_BC,file=paste(main_path,paths[ipath],'/cor_yd_BC_',max_time_upper,'h.txt',sep=''),row.names=F,col.names=F)	
	rm(cor_yd_BC)
...
#save the updated parameter sets to text files
	write.table(t(new_BC_par),file=paste(main_path,paths[ipath],'/BC_Post_samples_',max_time_upper,'h.txt',sep=''),row.names=F,col.names=F)		
}
