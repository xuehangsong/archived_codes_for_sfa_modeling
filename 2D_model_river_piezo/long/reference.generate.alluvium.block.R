...
## obs.well.x = rep(x[river.bank.x[which.min(abs(x[river.bank.x]-120.))]],4)
## obs.well.depth = c(0.075,0.125,0.375,0.425)
## obs.well.z = z[river.bank.z[which.min(abs(x[river.bank.x]-120.))]]+0.025-obs.well.depth

obs.well.x = rep(x[river.bank.x[which.min(abs(x[river.bank.x]-120.))]],4)
obs.well.x = c(obs.well.x,rep(x[river.bank.x[which.min(abs(x[river.bank.x]-112.))]],4))
obs.well.depth = rep(c(0.075,0.125,0.375,0.425),2)
obs.well.z = z[river.bank.z[which.min(abs(x[river.bank.x]-120.))]]+0.025-obs.well.depth[1:4]
obs.well.z = c(obs.well.z,z[river.bank.z[which.min(abs(x[river.bank.x]-112.))]]+0.025-obs.well.depth[5:8])
...
#for (block.x in seq(106,140,4)) {
#for (block.x in seq(104,142,4)) {
## for (block.x in seq(120,120,4)) {        

##     for (block.dx in seq(-1,1,0.02)) {
##         selected.bank.x = river.bank.x[which.min(abs(x[river.bank.x]-block.x-block.dx))]
##         selected.bank.z = river.bank.z[which.min(abs(x[river.bank.x]-block.x-block.dx))]
##         selected.bank.z = selected.bank.z-seq(-10,11)
##         material.ids[selected.bank.x,1,selected.bank.z][which(material.ids[selected.bank.x,1,selected.bank.z]>0)] = 11
##     }
## }
...
for (block.x in c(112,120)) {        

    for (i in 1:10){
    
        for (block.dx in seq(-0.2*i,0.2*i,0.02)) {
            selected.bank.x = river.bank.x[which.min(abs(x[river.bank.x]-block.x-block.dx))]
            selected.bank.z = river.bank.z[which.min(abs(x[river.bank.x]-block.x-block.dx))]
            selected.bank.z = selected.bank.z-seq(1+round(0.5*18*i/10),19-round(0.5*18*i/10))
            material.ids[selected.bank.x,1,selected.bank.z][which(material.ids[selected.bank.x,1,selected.bank.z]>0)] = 11
        }
...
material.ids.plot[,330:400][material.ids.plot[,330:400]==0] = -2 #105.0642
material.ids.plot[,288:330][material.ids.plot[,288:330]==0] = -1 #107.2273

jpegfile="./figures/reference_material_2d.jpg"
jpeg(jpegfile,width=6.5,height=2.6,units='in',res=600,quality=100)
...
#      ylab='Elevation (m)',
      ...
      col=c("lightblue1","blue","lightgoldenrod","grey","green","grey"),
#      cex.lab=0.8,
#      cex.axis=0.8,
      asp=1.0,
      axes=FALSE,
