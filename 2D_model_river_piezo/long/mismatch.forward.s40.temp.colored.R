...
{
    obs.value = (spc.temp.value[["S40"]]-spc.temp.value[["SWS-1"]])/(spc.temp.value[["2-3"]]-spc.temp.value[["SWS-1"]])
}
...
simu.ensemble = tracer.ensemble[,4,]
...
jpeg(paste("figures/s40.1.temp.forward.jpg",sep=''),width=14,height=8.4,units='in',res=200,quality=100)
...
lines(interp.time[compare.time],simu.ensemble[56,compare.time],col='orangered',lwd=5)
...
legend(x=start.time,y= 1.9,c("Inland WL","River WL","S40 GC","Simulated GC"),
       cex=1.65,
       lty=c(1,1,NA,1),
       pch=c(NA,NA,16,NA),       
