...
jpeg(paste("figures/all.temperature.jpg",sep=''),width=14,height=10,units='in',res=200,quality=100)
...
lines(interp.time,do.temp.value[["S10"]],col='green',lwd=2)
lines(interp.time,do.temp.value[["S40"]],col='red',lwd=2)


axis(side = 1,at = time.ticks.minor,label=rep("",length(time.ticks.minor)),cex.axis=1.8)
axis(side = 4,at=c(5,10,15,20),col.axis='black',col='black',line=0,las=2,cex.axis=1.8)
mtext(side = 4,at = 13 ,text=bquote("Temperature ("^o*"C)"),col='black',line=5.5,cex=1.8)
...
lines(interp.time,temp.value[["-64cm"]],col='red',lwd=2)
lines(interp.time,temp.value[["-24cm"]],col='green',lwd=2)
lines(interp.time,temp.value[["-4cm"]],col='cyan',lwd=2)

lines(interp.time,temp.value[["+46cm"]],col='purple',lwd=2)
lines(interp.time,temp.value[["+16cm"]],col='orange',lwd=2)



axis(side = 1,at = time.ticks.minor,label=rep("",length(time.ticks.minor)),cex.axis=1.8)
axis(side = 2,at=c(5,10,15,20),col.axis='black',col='black',line=0,las=2,cex.axis=1.8)
mtext(side = 2,at = 13 ,text=bquote("Temperature ("^o*"C)"),col='black',line=5.5,cex=1.8)
...
{
    lines(rep(interp.time[i.line]+900,2),c(25,50),lty=2, lwd=2,col="orange")
}
...
for (i.line in line.mark)
{
    lines(rep(interp.time[i.line]+900,2),c(25,50),lty=2,lwd=2,col="cyan")
}
...
legend(mean(c(start.time,end.time))-24*8.5*3600,54,
...
legend(mean(c(start.time,end.time))-24*8.5*3600,24,
       c("2-3","SWS-1"),#,"NRG","S10","S40"),
       cex=1.8,
       lty=c(1,1,1,1,1),
       col=c("black","blue","orange","green","red"),
...
legend(mean(c(start.time,end.time))-24*8.5*3600,22.5,
