...
jpeg(paste("figures/all.diff.2.temperature.jpg",sep=''),width=14,height=10,units='in',res=200,quality=100)
...
lines(interp.time,spc.temp.value[["S10"]]-spc.temp.value[["SWS-1"]],col='green',lwd=2)
lines(interp.time,spc.temp.value[["S40"]]-spc.temp.value[["SWS-1"]],col='red',lwd=2)
lines(interp.time,spc.temp.value[["NRG"]]-spc.temp.value[["SWS-1"]],col='orange',lwd=2)
...
{
    lines(rep(interp.time[i.line]+900,2),c(-2,16),lty=2, lwd=2,col="orange")
}
...
for (i.line in line.mark)
{
    lines(rep(interp.time[i.line]+900,2),c(-2,16),lty=2,lwd=2,col="cyan")
}
...
legend(mean(c(start.time,end.time))-24*8.5*3600,18,
...
legend(mean(c(start.time,end.time))-24*8.5*3600,6,
       c("NRG (EC sensor)","S10 (EC sensor)","S40 (EC sensor)"),
       cex=1.8,
       lty=c(1,1,1,1,1),
