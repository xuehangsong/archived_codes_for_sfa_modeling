...
jpeg(jpegfile,width=6,height=3.6,units='in',res=500,quality=100)
...
axis(side = 1,at=time.ticks,labels = format(time.ticks,format="%Y-%m"),mgp=c(1,0.7,0))
...
box()
legend(start.time,22.5,lty=1,pch=NA,c("-0.64m","-0.24m","-0.04m"),
       col=c("red","orange","green"),horiz=TRUE,bty="n")

legend(start.time,21,lty=1,pch=NA,c("River","Inland"),
...
for (isection in c("A","B","C","D","E"))
    {
        ...
        legend(start.time,20,as.character(ibutton.depth[isection,ibutton.list[[isection]]]),col=color[ibutton.list[[isection]]],lty=1,pch=NA)
        lines(index(inland.data),inland.data[,2],col='black')
