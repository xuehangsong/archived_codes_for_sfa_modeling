...
cenkf.mc.input <- function()
{
    ...
    pflotranin[lindex+1] = paste("    TIMES sec ",obs.time[itime-1])
    ...
    if(itime<=3)
    {
        ...
        pflotranin[lindex] = paste(" RESTART 1dthermal-",sprintf("%.4f",obs.time[itime-2]),"sec.chk",sep="")        
    }

    lindex = grep("FINAL_TIME",pflotranin)        
    pflotranin[lindex] = paste("  FINAL_TIME",obs.time[itime],"sec")

    lindex = grep("SNAPSHOT_FILE",pflotranin)        
    pflotranin[lindex+1] = paste("   TIMES sec",obs.time[itime])
...
cenkf.mc.output <- function()
{
    ...
    for (ireaz in 1:nreaz)
    {
        simu.ensemble[ireaz,] = h5read(paste(mc_dir,ireaz,"/1dthermal.h5",sep=''),
                               paste("Time:",sprintf("%12.5E",obs.time[itime]),"s/Temperature [C]"))[obs.cell]
    }
