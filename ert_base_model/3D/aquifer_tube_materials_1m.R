...
load("results/aquifer_tube_bath_unit.r")
...
for (iz in 1:nz)
{
    material.ids[,,iz][which(cells_unit<=z[iz])] = 1        
    material.ids[,,iz][which(cells_bath<=z[iz])] = 0
}
...
jpeg('figures/river_face_3d.jpg',width=8,height=7.4,units='in',res=300,quality=100)
...
for (iface in 1:length(face.ids.final))
{
    ...
    jz = which.min(abs(z-(z[iz]-1)))                         ##make the allvium depth equaling to 2m
    material.ids[ix,iy,jz:iz] = 5
}

fname = "ert_model/aquifer_tube_material_1m.h5"
...
H5close()
save(list=ls(),file="results/material.ids_1m.r")
