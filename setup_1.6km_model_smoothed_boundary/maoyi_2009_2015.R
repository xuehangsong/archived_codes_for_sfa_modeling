rm(list=ls())
load("results/mass.data.xts_2009_2015.r")

start.time = as.POSIXct("2009-01-01 00:00:00",tz="GMT",format="%Y-%m-%d %H:%M:%S")
...
DatumH_River = cbind(simu.time,
                     rep(coord.data["321",1],ntime),
                     rep(coord.data["321",2],ntime),
                     mass.level[which(slice.list=="321"),])
...
write.table(DatumH_River,file=paste('DatumH_River_2009_2015.txt',sep=""),col.names=FALSE,row.names=FALSE)
write.table(Gradients_River,file=paste("Gradients_River_2009_2015.txt",sep=''),col.names=FALSE,row.names=FALSE)
