...
model.coord = array(rep(0,8),c(4,2))
model.coord[,1] = c(0,400,400,0)
model.coord[,2] = c(0,0,400,400)

proj.coord = array(rep(0,8),c(4,2))
proj.coord[,1] = x.origin+model.coord[,1]*cos(angle)-model.coord[,2]*sin(angle)
proj.coord[,2] = y.origin+model.coord[,1]*sin(angle)+model.coord[,2]*cos(angle)

write.table(proj.coord,file = "data/model.domain.dat",col.names = FALSE,row.names = FALSE )    


model.coord = array(rep(0,8),c(4,2))
model.coord[,1] = c(-450,450,450,-450)
model.coord[,2] = c(-800,-800,800,800)

proj.coord = array(rep(0,8),c(4,2))
proj.coord[,1] = x.origin+model.coord[,1]*cos(angle)-model.coord[,2]*sin(angle)
proj.coord[,2] = y.origin+model.coord[,1]*sin(angle)+model.coord[,2]*cos(angle)

write.table(proj.coord,file = "data/model.domain.dat",append=TRUE,col.names = FALSE,row.names = FALSE )    
